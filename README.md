...
- **題庫管理**：首頁可建立、重新命名（點擊名稱即可內聯編輯）、複製、刪除題庫
- **出題介面**：網頁式題庫設計，支援文字與圖片題目（題目及每個選項皆可上傳圖片），最多 256 題，可拖曳排序，新題目預設 10 秒
- **等待畫面自訂圖片**：出題者可在題庫設計頁面上傳等待畫面圖片，顯示於玩家加入後等待遊戲開始的畫面（未上傳則顯示預設十字架圖示）
- **即時遊戲**：Socket.io 驅動，毫秒級同步玩家作答狀態；玩家短暫斷線（30 秒內）可自動重連恢復作答，無需重新輸入暱稱
...
| 狀態 | 顯示畫面 | 主持人畫面 | 玩家畫面 |
|------|----------|------------|----------|
| LOBBY | QR Code + 遊戲名稱 | 玩家列表 + 開始按鈕 | 輸入暱稱表單 → 加入後顯示等待畫面（自訂圖片或十字架）|
...
| PUT | `/api/quizzes/:id` | 更新題庫（含重新命名）|
| DELETE | `/api/quizzes/:id` | 刪除題庫 |
| POST | `/api/quizzes/:id/duplicate` | 複製題庫（標題加上「複製」）|
...
|------|--------|------------|----------|-----------|-------|
| 後端 | 151 | 96.38% | 88.69% | 98.91% | 98.20% |
| 前端 | 227 | 97.16% | 91.93% | 90.24% | 97.16% |
